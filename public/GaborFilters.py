# -*- coding: utf-8 -*-
"""
Created on Fri Dec 14 18:09:40 2018
    ...
    for theta0 in theta:
        for sigma0 in sigma:
            for frequency0 in frequency:
                kernel = (gabor_kernel(frequency0, theta=theta0,
                                       sigma_x=sigma0, sigma_y=sigma0))
...
def GaborFilterBank3D(theta=None, sigma=None, frequency=None):
    ...
    for theta0 in theta[0, :]:
        for theta1 in theta[1, :]:
            for sigma0 in sigma:
                for frequency0 in frequency:
                    kernel = gabor_fn3D([sigma0, sigma0, sigma0], [theta0, theta1], frequency0)
...
def gabor_fn3D(sigma, theta, frequency):
    ...
    (x, y, z) = np.meshgrid(np.arange(xmin, xmax + 1), np.arange(ymin, ymax + 1), np.arange(zmin, zmax + 1))

    # Rotation 
    x_theta = x * np.cos(theta1) * np.cos(theta2) + y * np.sin(theta1) * np.cos(theta2) + z * np.sin(theta2)
    y_theta = -x * np.sin(theta1) + y * np.cos(theta1)
    z_theta = -x * np.cos(theta1) * np.sin(theta2) - y * np.sin(theta1) * np.sin(theta2) + z * np.cos(theta2)

    gb_exp = np.exp(-.5 * (x_theta ** 2 / sigma_x ** 2 + y_theta ** 2 / sigma_y ** 2 + z_theta ** 2 / sigma_z ** 2))
...
def gabor_support(sigma, theta):
    ...
    xmax = max(abs(nstds * sigma_x * np.cos(theta1) * np.cos(theta2)), abs(nstds * sigma_y * np.sin(theta1)))
    xmax = max(xmax, abs(nstds * sigma_z * np.cos(theta1) * np.sin(theta2)))
    xmax = np.ceil(max(1, xmax))

    ymax = max(abs(nstds * sigma_x * np.sin(theta1) * np.cos(theta2)), abs(nstds * sigma_y * np.cos(theta1)))
    ymax = max(ymax, abs(nstds * sigma_z * np.sin(theta1) * np.sin(theta2)))
    ymax = np.ceil(max(1, ymax))

    zmax = max(abs(nstds * sigma_x * np.sin(theta2)), abs(nstds * sigma_z * np.cos(theta2)))
