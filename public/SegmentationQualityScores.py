...
Quality Measures of an automatic Segmentation_Validation computed from
...
Two types of measures are implemented:
    ...
    in terms of distance between Segmentation_Validation and ground truth.
...
Liver Segmentation_Validation From CT Datasets, IEEE Trans Med Imag, 28(8),2009
"""
import numpy as np
...
def DICE(Seg, GT):
    ...
    INPUT:
    1> Seg: Binary ndarray of Segmentation_Validation
...
def VOE(Seg, GT):
    ...
    INPUT:
        ...
        2> GT:  Binary ndarray of true object
    """
    voe = 100 * (1 - (np.sum(Seg * GT)) / (np.sum(Seg) + np.sum(GT)-np.sum(Seg * GT)))
    return voe
