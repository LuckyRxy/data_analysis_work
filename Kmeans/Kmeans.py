...
sig = 1  # sigma of gaussian filter
...
# Mask
NiiPath = r'D:\Project\PythonProject\data_analysis\resources\VOIs\nodule_mask\LIDC-IDRI-0001_R_1.nii.gz'
NiiPath = NiiPath.replace('\\', '/')
niiROIGT, _ = readNifty(NiiPath)
...
ims = []
imMasks = []
for i in range(niiROI.shape[2]):
    im = niiROI[:,:,i]
    im = (im - im.min()) / (im.max() - im.min())
    # laplace_result = laplace(im)
    imMask = niiMask[:,:,i]
    ims.append(im)
    imMasks.append(imMask)
ims = np.dstack(ims)
...
niiROISegOpen = Morpho.binary_opening(ims, se)
